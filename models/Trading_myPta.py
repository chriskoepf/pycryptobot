...
#        self.addVWMA(8,True)
#        self.addVWMA_Signals(8,True)
#        self.addVWMA_Signals(20,True)
#        self.addATR()
#        self.addRSI(14,5,True) # add MA period to add Moving Average and True to add pcnt of change
#        self.addRSI()
#        self.addATR_RSI(10,14,14,5,"ema",5,"wma",True)
#        self.addATR_RSI(14,14,14,5,"wma",18,"wma",True)
        self.addATR_RSRL(14,14,use_wicks=True,add_cross=True)
#        self.addRSI_ATR_RSRL_signals(14,14,14,18,"wma",30,70,True,True)
        self.addTSI(14,10,5,"ema",True)
#        self.addMT4MT5(13,7,8,True)
#        self.addRVI(8,"wma",5,"wma",20,True)
...
#        self.addWilliamsR() # default period is 20, add an integer to change default
        ...
        self.addVWMACD(12,26,5,True)
        self.addOBV("sma",5,True) # ma_indicator, ma_period, addPC - defaults sma,5,False
#        self.addOBVMACD(12,26,9,True)
...
#        self.addWaveTrend(9,12,True) # add n1=x for Channel Length (default=10)
    ...
    def addHalfCandle(self) -> None:
        '''Add 50%/Half candle value to dataframe - used in some signals'''

        self.df['halfcandle'] = np.where(
            self.df['open'] < self.df['close'],
            (self.df['close'] - self.df['open'])/2 + self.df['open'],
            (self.df['open'] - self.df['close'])/2 + self.df['close']
        )
...
#            high=self.df["high"], low=self.df["low"], close=self.df["close"], length=interval, mamode="rma", talib=self.talib
            high=self.df["high"], low=self.df["low"], close=self.df["close"], length=interval, talib=self.talib
        )
    ...
    def addATR_RSI(
        ...
        atr_interval: int = 14,
        rsi_interval: int = 14,
        rsiatr_interval: int = 14,
        sm_interval: int = 3,
        sm_indicator: str = "sma",
        ma_interval: int = 5,
        ma_indicator: str = "sma",
        addPC: bool = False
    ) -> None:
        """Adds ATR adjusted and smoothed RSI"""

        if (
            atr_interval > self.total_periods
            or atr_interval < 5
            or atr_interval > 200
            or rsi_interval > self.total_periods
            or rsi_interval < 5
            or rsi_interval > 200
        ):
            raise ValueError("an interval is out of range")

        if len(self.df) < atr_interval or len(self.df) < rsi_interval:
            raise Exception("Data range too small.")

        df = pd.DataFrame()

        if "atr" + str(atr_interval) not in self.df:
            self.addATR(atr_interval)

        if "rsi" + str(rsi_interval) not in self.df:
            self.addRSI(rsi_interval,ma_interval,ma_indicator,addPC)

        # adjust RSI by multiplying by ATR
        rsi_atr_adj = self.df["atr" + str(atr_interval)] * self.df["rsi" + str(rsi_interval)]
        # smooth the jagged RSI line for - if not smoothing, just use value of 1 for interval
        rsi_atr_smooth = ta.ma(sm_indicator, rsi_atr_adj, length=sm_interval, talib=self.talib)
        # rerun RSI with ma smoothed values
        df["rsi_atr"] = ta.rsi(rsi_atr_smooth, length=rsiatr_interval, talib=self.talib).fillna(0)
        # get moving average of the adjust RSI
        df["rsi_atr_ma"] = ta.ma(ma_indicator, df["rsi_atr"], length=ma_interval, talib=self.talib).fillna(0)
        # add percentage of change for RSI line
        if addPC is True:
            df["rsi_atr_pc"] = self.calcPC(df["rsi_atr"])
    ...
    def addATR_RSRL(
        ...
        if "atr" + str(atr_period) not in self.df:
            self.addATR(atr_period)

        df = pd.DataFrame()
        # add close, high, low to df - didn't work right if using original df
        df['close'] = self.df['close']
        df['open'] = self.df['open']
        df['high'] = self.df['high']
        df['low'] = self.df['low']

        # add atr (from main df) * 2 to the df
        dev = 2 * self.df['atr' + str(atr_period)]

        basis = ta.sma(df['close'], atr_period, talib=self.talib)

        # Calculate upper and lower bounds
        upper = basis + dev
        lower = basis - dev

        # Calculate bbr and bbe
        bbr = (df['close'] - lower) / (upper - lower)
        bbe = ta.ema(close=bbr, length=ma_period, talib=self.talib)

        # Find values for up and bt
        up = np.where((bbe.shift(1) > bbe) & (bbe.shift(2) < bbe.shift(1)), bbe, np.nan)
        bt = np.where((bbe.shift(1) < bbe) & (bbe.shift(2) > bbe.shift(1)), bbe, np.nan)

        # Find topH and bottomL
        if use_wicks is False:
            topH = np.where(np.isnan(up), np.nan, df['close'].rolling(window=3).max().shift(1).squeeze())
            bottomL = np.where(np.isnan(bt), np.nan, df['close'].rolling(window=3).min().shift(1).squeeze())
        else:
            topH = np.where(np.isnan(up), np.nan, df['high'].rolling(window=3).max().shift(1).squeeze())
            bottomL = np.where(np.isnan(bt), np.nan, df['low'].rolling(window=3).min().shift(1).squeeze())

        # Calculate tf and bf
        df['tf'] = np.where(np.isnan(topH), np.nan, topH)
        df['bf'] = np.where(np.isnan(bottomL), np.nan, bottomL)
        df['tf'].fillna(method='ffill', inplace=True)
        df['bf'].fillna(method='ffill', inplace=True)
        df['tf'] = df['tf'].fillna(0)
        df['bf'] = df['bf'].fillna(0)

        # do we add crossovers to dataframe?
        if add_cross:
            # we need half candle values
            if "halfcandle" not in self.df:
                self.addHalfCandle()
            df['halfcandle'] = self.df['halfcandle']

            # half candle crosses resistance line upward/downward
            df['candle_xresist'] = np.where(
                (df['halfcandle'].shift(1) <= df['tf']) & (df['halfcandle'] > df['tf']),
                "up",
                np.where(
                    (df['halfcandle'].shift(1) >= df['bf']) & (df['halfcandle'] < df['bf']),
                    "down",
                    0
                )
            )
            # drop half candle before concat
            df = df.drop("halfcandle", axis=1)

            # set the last direction rsi crossed the ma
            df['candle_xresist_last'] = np.where(
                ( # set last where xma is up or down
                    (df['candle_xresist'] == "up").astype(bool)
                    | (df['candle_xresist'] == "down").astype(bool)
                ),
                df['candle_xresist'], # xma value to use
                np.nan # all other rows get set to nan
            )
            df['candle_xresist_last'].fillna(method='ffill', inplace=True) # fill all nan with last non-nan entry
            df['candle_xresist_last'] = df['candle_xresist_last'].fillna(0) # now fill leftover nan with 0

        # drop columns already in main df
        df = df.drop(["close", "open", "high", "low"], axis=1)
#        print(df)
        # append new columns to main df
        self.df = pd.concat([self.df, df], axis=1)

    def addRSI_ATR_RSRL_signals(
        self,
        atr_period: int = 14,
        atrma_period: int = 14,
        rsi_period: int = 14,
        rsima_period: int = 20,
        rsima_indicator: str = "sma",
        lwr_band: int = 30,
        up_band: int = 70,
        rsi_pc: bool = False,
        atr_rsi: bool = False
    ) -> None:
        """Adds ATR-RSRL with RSI trade signals"""

        # if using this signal function with ATR adjusted RSI instead of standard RSI,
        # you MUST add ATR_RSI to the dataframe first because it has additional variables
        # not specified here

        if "tf" not in self.df or "bf" not in self.df or "candle_xresist" not in self.df:
            self.addATR_RSRL(atr_period, atrma_period, use_wicks=False, add_cross=True)

        if atr_rsi is True and "rsi_atr" not in self.df:
            raise Exception(
                "ATR adjusted RSI is not in dataframe. It is required if option enabled for RSI_ATR_RSRL_signals."
            )
        elif "rsi" + str(rsi_period) not in self.df or "rsima" + str(rsima_period) not in self.df:
            self.addRSI(rsi_period,rsima_period,rsima_indicator)

        df = pd.DataFrame()

        # add columns from main df needed for comparisons
        if atr_rsi is True:
            df['rsi'] = self.df['rsi_atr']
            df['rsima'] = self.df['rsi_atr_ma']
        else:
            df['rsi'] = self.df['rsi' + str(rsi_period)]
            df['rsima'] = self.df['rsima' + str(rsima_period)]

        # rsi cross its moving average upward/downware
        df['rsi_xma'] = np.where(
            (df['rsi'].shift(1) <= df['rsima']) & (df['rsi'] > df['rsima']),
            "up",
            np.where(
                (df['rsi'].shift(1) >= df['rsima']) & (df['rsi'] < df['rsima']),
                "down",
        ...
        # rsi cross lower band upward or upper band downward
        df['rsi_xlwr'] = np.where(
            (df['rsi'].shift(1) <= lwr_band) & (df['rsi'] > lwr_band),
            "up",
            np.where(
                (df['rsi'].shift(1) >= lwr_band) & (df['rsi'] < lwr_band),
                "down",
        ...
        df['rsi_xupr'] = np.where(
            (df['rsi'].shift(1) >= up_band) & (df['rsi'] < up_band),
            "down",
            np.where(
                (df['rsi'].shift(1) <= up_band) & (df['rsi'] > up_band),
                "up",
        ...
        # set the last direction rsi crossed lower band
        df['rsi_xlwr_last'] = np.where(
            ( # when rsi crosses ma, upper or lower band, we reset last to xlwr value
                (df['rsi_xma'] == "down").astype(bool) 
                | (df['rsi_xupr'] == "up").astype(bool)
                | (df['rsi_xupr'] == "down").astype(bool)
                | (df['rsi_xlwr'] == "up").astype(bool)
                | (df['rsi_xlwr'] == "down").astype(bool)
            ), 
            df['rsi_xlwr'], # xlwr value to use
            np.nan # all other rows get set to nan
        )
        df['rsi_xlwr_last'].fillna(method='ffill', inplace=True) # fill all nan with last non-nan entry

        # set the last direction rsi crossed upper band
        df['rsi_xupr_last'] = np.where(
            ( # when rsi crosses upper or lower band, we reset last to xupr value
                (df['rsi_xupr'] == "up").astype(bool)
                | (df['rsi_xupr'] == "down").astype(bool)
                | (df['rsi_xlwr'] == "up").astype(bool)
                | (df['rsi_xlwr'] == "down").astype(bool)
            ), 
            df['rsi_xupr'], # xupr value to use
            np.nan # all other rows get set to nan
        )
        df['rsi_xupr_last'].fillna(method='ffill', inplace=True) # now fill all nan with last non-nan entry

        # set the last direction rsi crossed the ma
        df['rsi_xma_last'] = np.where(
            ( # set last where xma is up or down
                (df['rsi_xma'] == "up").astype(bool)
                | (df['rsi_xma'] == "down").astype(bool)
            ),
            df['rsi_xma'], # xma value to use
            np.nan # all other rows get set to nan
        )
        df['rsi_xma_last'].fillna(method='ffill', inplace=True) # fill all nan with last non-nan entry

        # drop unnecessary or columns already in main df
        df = df.drop(["rsi", "rsima"], axis=1)
        # append new columns to main df
        self.df = pd.concat([self.df, df], axis=1)

#        print(df.tail(50))

    def addTSI(self, long: int = 25, short: int = 13, signal: int = 13, ma: str = "ema", addPC: bool = False) -> None:
        """Adds the True Strength Index (TSI)"""

        if not isinstance(long, int):
            raise TypeError("slow parameter is not intervaleric.")

        if long > self.total_periods:
            raise ValueError("slow range is out of range")
        ...
        df = ta.tsi(
            close=self.df["close"], fast=long, slow=short, signal=signal, mamode=ma, talib=self.talib
        )

        self.df["tsi"] = df["TSI" + "_" + str(long) + "_" + str(short) + "_" + str(signal)].fillna(0)
        self.df["tsi_signal"] = df["TSIs" + "_" + str(long) + "_" + str(short) + "_" + str(signal)].fillna(0)

        if addPC is True:
            self.df["tsi_pc"] = self.calcPC(self.df["tsi"])

    def addMT4MT5(self, periodK: int = 13, periodD: int = 3, smoothK: int = 4, addPC: bool = False) -> None:
        """Adds MT4 MT5"""

        if periodK > self.total_periods:
            raise ValueError("periodK is out of range")

        if len(self.df) < periodK:
            raise Exception("Data range too small.")
        ...
        df["k"] = ta.wma(df["STOCHk_"+str(periodK)+"_3_3"], smoothK, talib=self.talib)
        df["d"] = ta.wma(df["k"], periodD, talib=self.talib)

        df["k"] = df["k"].fillna(0)
        df["d"] = df["d"].fillna(0)

        if addPC is True:
            df["k_pc"] = self.calcPC(df["k"])

        df = df.drop(["STOCHk_"+str(periodK)+"_3_3", "STOCHd_"+str(periodK)+"_3_3"], axis=1)
        self.df = pd.concat([self.df, df], axis=1)
    ...
    def addRVI(
        ...
        """Adds RVI with optional smoothing"""

        if (
            rvi_interval > self.total_periods
            or rvi_interval < 5
            or rvi_interval > 200
        ):
            raise ValueError("an rvi interval is out of range")

        if len(self.df) < rvi_interval or len(self.df) < sm_interval:
            raise Exception("RVI data range too small.")

        df = pd.DataFrame()

        df["rvi"] = ta.rvi(self.df["close"], high=None, low=None, length=rvi_interval, talib=self.talib)
        df["rvi"] = df["rvi"].fillna(0)

        if sm_interval > 0:
            df["rvi_sm"] = ta.ma(sm_indicator, df["rvi"], length=sm_interval, talib=self.talib)
            df["rvi_ma"] = ta.ma(ma_indicator, df["rvi_sm"], length=ma_interval, talib=self.talib)
            self.df["rvi"] = df["rvi_sm"].fillna(0)
        else:
            df["rvi_ma"] = ta.ma(ma_indicator, df["rvi"], length=ma_interval, talib=self.talib)
            self.df["rvi"] = df["rvi"]

        self.df["rvi_ma"] = df["rvi_ma"].fillna(0)

        if addPC is True:
            self.df["rvi_pc"] = self.calcPC(self.df["rvi"])

    def calcPC(self,indicator) -> DataFrame:
        '''Will return the percent of change for specified indicator for 1 period'''

        try:
            pc = round((indicator - indicator.shift()) / abs(indicator) * 100, 2)
            pc = pc.fillna(0)
            return pc
        except Exception as err:
            raise Exception("calcPC function error: " + err)
    ...
    def addVWMA(self, period: int = 10, addPC: bool = False) -> None:
        """Adds the Volume Weighted Moving Average (VWMA) to the DataFrame"""

        self.df["vwma" + str(period)] = ta.vwma(self.df["close"], self.df["volume"], length=period, talib=self.talib)

        if addPC is True:
            self.df["vwma" + str(period) + "_pc"] = self.calcPC(self.df["vwma" + str(period)]).fillna(0)

    def addVWMA_Signals(self, period: int = 10, addPC: bool = False) -> None:
        """Adds VWMA trade signals"""

        # if wanting VWMA percent of change in the dataframe,
        # make sure you also use addVWMA in addition to this function
        # as the default used is without percent of change

        if "vwma" + str(period) not in self.df:
            self.addVWMA(period, addPC)
        ...
        # add columns from main df needed for comparisons
        df['vwma'] = self.df['vwma' + str(period)]
        df['halfcandle'] = self.df['halfcandle']

        # price crosses vwma upward/downward
        df['vwma' + str(period) + 'cross'] = np.where(
            (df['halfcandle'].shift(1) <= df['vwma']) & (df['halfcandle'] > df['vwma']),
            "up",
            np.where(
                (df['halfcandle'].shift(1) >= df['vwma']) & (df['halfcandle'] < df['vwma']),
                "down",
        ...
        # set a column for which cross happened last
        df['vwma' + str(period) + 'cross_last'] = np.where(
            ( # set last where xma is up or down
                (df['vwma' + str(period) + 'cross'] == "up").astype(bool)
                | (df['vwma' + str(period) + 'cross'] == "down").astype(bool)
            ),
            df['vwma' + str(period) + 'cross'], # xma value to use
            np.nan # all other rows get set to nan
        )
        df['vwma' + str(period) + 'cross_last'].fillna(method='ffill', inplace=True) # fill all nan with last non-nan entry

        # drop unnecessary or columns already in main df
        df = df.drop(["vwma","halfcandle"], axis=1)
        # append new columns to main df
        self.df = pd.concat([self.df, df], axis=1)
...
#        print(df.tail(50))
    ...
    def ta_MACD(self, fast: int=12, slow: int=26, sig: int=9) -> DataFrame:
        ...
        df = pd.DataFrame()
        # pandas-ta version did not allow specifying which ma signal to use
...
#        df["hist"] = df["MACDh_8_21_5"].fillna(0)
        ...
        df["fast_ma"] = ta.ema(close=self.df["close"], length=fast, talib=self.talib)
        df["slow_ma"] = ta.ema(close=self.df["close"], length=slow, talib=self.talib)
        df["macd"] = df["fast_ma"] - df["slow_ma"]
        df["signal"] = ta.ema(close=df["macd"], length=sig, talib=self.talib)
    ...
    def addVWMACD(
        ...
        """Calculates the Volume Weighted Moving Average Convergence Divergence VWMACD)"""

        if len(self.df) < slow:
            raise Exception("VWMACD Data range too small.")

        df = pd.DataFrame()

        fastMA = ta.ema(self.df["volume"]*self.df["close"], fast)/ta.ema(self.df["volume"], fast)
        slowMA = ta.ema(self.df["volume"]*self.df["close"], slow)/ta.ema(self.df["volume"], slow)
        df["vwmacd"] = fastMA - slowMA
        df["vwmacd_sig"] = ta.ema(df["vwmacd"], sig)
        df["vwmacd_hist"] = df["vwmacd"]  - df["vwmacd_sig"]

        df["vwmacd"] = df["vwmacd"].fillna(0)
        df["vwmacd_sig"] = df["vwmacd_sig"].fillna(0)
        df["vwmacd_hist"] = df["vwmacd_hist"].fillna(0)

        if addPC:
            df["vwmacd_pc"] = self.calcPC(df["vwmacd"]).fillna(0)
    ...
    def addOBV(self, ma_indicator: str = "sma", ma_period: int = 5, addPC: bool = False) -> None:

        """Add the On-Balance Volume (OBV) to the DataFrame"""

        df = pd.DataFrame()

        df["obv"] = ta.obv(close=self.df["close"], volume=self.df["volume"].fillna(0), talib=self.talib).fillna(0)
        df['obvsm'] = ta.ma(ma_indicator, df["obv"], length=ma_period, talib=self.talib).fillna(0)

        if addPC is True:
            df["obv_pc"] = self.calcPC(df["obv"]).fillna(0)
            df["obvsm_pc"] = self.calcPC(df["obvsm"]).fillna(0)

        self.df = pd.concat([self.df, df], axis=1)

    def addOBVMACD(self, fast: int=12, slow: int=26, signal: int=9, addPC: bool=False) -> None:

        df = pd.DataFrame()

        df = ta.macd(close=(ta.obv(close=self.df["close"], volume=self.df["volume"], talib=self.talib)), fast=fast, slow=slow, signal=signal, talib=self.talib)
        self.df["obvmacd"] = df["MACD_" + str(fast) + "_" + str(slow) + "_" + str(signal)].fillna(0)
        self.df["obvmacdsig"] = df["MACDs_" + str(fast) + "_" + str(slow) + "_" + str(signal)].fillna(0)
        self.df["obvmacdhist"] = df["MACDh_" + str(fast) + "_" + str(slow) + "_" + str(signal)].fillna(0)

        if addPC is True:
            self.df["obvmacd_pc"] = self.calcPC(self.df["obvmacd"])
    ...
    def addRSI(
        ...
        # add RSI with sepcified period length
        df["rsi" + str(period)] = ta.rsi(close=df['close'], length=period, talib=self.talib)
        df["rsi" + str(period)] = df["rsi" + str(period)].fillna(0)
        # add move average for RSI
        df["rsima" + str(ma_period)] = ta.ma(ma_indicator, df["rsi" + str(period)], length=ma_period, talib=self.talib).fillna(0)

        # if wanting percentage of change to DF, add it.
        if addPC is True:
            df["rsi" + str(period) + "_pc"] = self.calcPC(df["rsi" + str(period)]).fillna(0)
            df["rsima" + str(ma_period) + "_pc"] = self.calcPC(df["rsima" + str(ma_period)]).fillna(0)
    ...
    def addWaveTrend(self, n1: int=10, n2: int=21, addPC: bool = False) -> None:
        """ Adds Wave Trend to the data frame """
       
        df = pd.DataFrame()

        ap = ta.hlc3(high=self.df["high"], low=self.df["low"], close=self.df["close"], talib=self.talib)
